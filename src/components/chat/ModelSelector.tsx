...
export function ModelSelector({ disabled, value, onValueChange, models }: ModelSelectorProps) {
    ...
    return (
        <Select value={value} onValueChange={onValueChange} disabled={disabled}>
            <SelectTrigger className="w-auto gap-1 border-0 bg-transparent hover:bg-white/5 transition-colors px-2 py-1.5 h-auto focus:ring-0 focus:ring-offset-0 data-[state=open]:bg-white/5 rounded-xl group">
            ...
            <SelectContent align="start" className="w-[200px] bg-[#2a2a2a] border-border/30">
                <SelectItem
                    value="auto"
                    className="cursor-pointer focus:bg-white/10 text-sm my-0.5"
                >
                ...
                {models.map((model) => (
                    ...
                    >
                        <div className="flex items-center gap-2">
                            {model.id.includes('pro') && <Sparkles className="h-3 w-3 text-amber-400" />}
                            {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
